...
def load_function_data(function_id: int) -> tuple[np.ndarray, np.ndarray, str]:
    ...
    week3_dir = repo_root / "week-3" / "data" / f"function_{function_id}"
    ...
    for x_name, y_name in candidate_pairs:
        x_path = week3_dir / x_name
        y_path = week3_dir / y_name
        if x_path.exists() and y_path.exists():
            return np.load(x_path), np.load(y_path), f"week-3/data/function_{function_id} ({x_name}, {y_name})"
...
def nearest_distances(candidates: np.ndarray, observed: np.ndarray) -> np.ndarray:
    # Pairwise Euclidean distances; small data makes a dense matrix practical.
...
def refine_with_gradient_ascent(bundle: SurrogateBundle, start: np.ndarray, steps: int = 35, lr: float = 0.08) -> np.ndarray:
    x_curr = start.copy()
...
def propose_query(function_id: int) -> dict:
    ...
    if min_dist < 1e-5:
        # If optimization lands on an old point, use the farthest high-scoring candidate.
...
def write_outputs(results: list[dict]) -> tuple[Path, Path]:
    week4_root = Path(__file__).resolve().parents[1]
    submission_path = week4_root / "submission_round4.txt"
    diagnostics_path = week4_root / "doc" / "nn_strategy_diagnostics.md"

    submission_lines = [r["query_str"] for r in results]
    payload = "\n".join(submission_lines) + "\n"
    submission_path.write_text(payload, encoding="ascii")

    lines: list[str] = []
    lines.append("# Week 4 NN Strategy Diagnostics")
    lines.append("")
    for r in results:
        ...
        mse = r["nn_metrics"]
        lines.append(
            "- NN error diagnostics (lower is better): "
            f"train_mse={mse['nn_train_mse']:.6g}, "
...
def main() -> None:
    ...
    print("Week 4 NN-guided query summary")
    print("=" * 60)
    for r in results:
        mse = r["nn_metrics"]
    ...
    print(f"Wrote diagnostics file: {diagnostics_path}")
