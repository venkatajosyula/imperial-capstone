...
def load_function_data(function_id: int) -> tuple[np.ndarray, np.ndarray, str]:
    ...
    # Search priority: week-10 > week-9 > week-8 > week-7 > week-6 > week-5 > week-4 > week-3 > week-2
    search_dirs = [
...
def propose_query(function_id: int) -> dict:
    ...
    # Normalize cluster quality to [0, 1] so cluster trend can be blended with surrogate score.
    ...
    c_idx, c_dist = nearest_centroid_indices(candidates, clusters.centroids)
...
def write_outputs(results: list[dict]) -> tuple[Path, Path]:
    week11_root = Path(__file__).resolve().parents[1]
    submission_path = week11_root / "submission_round11_nn.txt"
    diagnostics_path = week11_root / "doc" / "cluster_strategy_justification.md"
    ...
    for r in results:
        ...
        lines.append(
            f"- Cluster trend cue: normalized cluster quality={r['target_cluster_quality']:.4f}; "
            "score blended surrogate prediction + exploration distance + cluster trend"
        )
        lines.append(
            "- Build on first ten rounds: query is selected from cumulative history and "
            "steered toward high-performing neighborhoods while preserving distance from exact repeats"
        )
        mse = r["nn_metrics"]
        lines.append(
...
def main() -> None:
    ...
    print("Week 11 cluster-aware NN query summary")
    print("=" * 60)
    for r in results:
        mse = r["nn_metrics"]
