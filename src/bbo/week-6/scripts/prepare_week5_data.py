"""
Parse week-6/inputs.txt and week-6/outputs.txt (cumulative 5-round data),
then append only the round-5 result to week-4's cumulative .npy files,
saving the result into week-5/data/function_X/.
...
def main() -> None:
    ...
    week5_data_root = repo_root / "week-5" / "data"

    raw_inputs = (week6_dir / "inputs.txt").read_text(encoding="utf-8").splitlines()
    outputs_lines = (week6_dir / "outputs.txt").read_text(encoding="utf-8").splitlines()
    ...
    # week-4/data already contains rounds 1-4 (14 points for 2D functions).
    # Only the last row (round 5) is new and must be appended.
    ...
    for fid in range(1, n_functions + 1):
        ...
        # Load existing cumulative data from week-4
        week4_dir = repo_root / "week-4" / "data" / f"function_{fid}"
        x_existing = np.load(week4_dir / "inputs.npy")
        y_existing = np.load(week4_dir / "outputs.npy")
        ...
        out_dir = week5_data_root / f"function_{fid}"
