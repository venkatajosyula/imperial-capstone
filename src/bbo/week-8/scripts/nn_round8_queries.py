...
def load_function_data(function_id: int) -> tuple[np.ndarray, np.ndarray, str]:
    ...
    # Search priority: week-7 > week-6 > week-5 > week-4 > week-3 > week-2 fallback
    search_dirs = [
...
def write_outputs(results: list[dict]) -> tuple[Path, Path]:
    week8_root = Path(__file__).resolve().parents[1]
    submission_path = week8_root / "submission_round8_nn.txt"
    diagnostics_path = week8_root / "doc" / "nn_strategy_diagnostics.md"
    diagnostics_path.parent.mkdir(parents=True, exist_ok=True)

    submission_lines = [r["query_str"] for r in results]
    submission_path.write_text("\n".join(submission_lines) + "\n", encoding="ascii")

    lines: list[str] = ["# Week 8 NN Strategy Diagnostics", ""]
    for r in results:
...
def main() -> None:
    ...
    print("Week 8 NN-guided query summary")
    print("=" * 60)
    for r in results:
        mse = r["nn_metrics"]
