"""
Parse week-7/inputs.txt and week-7/outputs.txt (cumulative 6-round data),
then append only the round-6 result to week-5's cumulative .npy files,
saving the result into week-6/data/function_X/.
...
def main() -> None:
    ...
    week7_dir = repo_root / "week-7"
    week6_data_root = repo_root / "week-6" / "data"

    raw_inputs = (week7_dir / "inputs.txt").read_text(encoding="utf-8").splitlines()
    outputs_lines = (week7_dir / "outputs.txt").read_text(encoding="utf-8").splitlines()
    ...
    # week-5/data already contains rounds 1-5 (15 points for 2D functions).
    # Only the last row (round 6) is new and must be appended.
    ...
    for fid in range(1, n_functions + 1):
        ...
        # Load existing cumulative data from week-5
        week5_dir = repo_root / "week-5" / "data" / f"function_{fid}"
        x_existing = np.load(week5_dir / "inputs.npy")
        y_existing = np.load(week5_dir / "outputs.npy")
        ...
        out_dir = week6_data_root / f"function_{fid}"
