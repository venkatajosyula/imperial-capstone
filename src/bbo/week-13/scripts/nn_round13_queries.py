...
def load_function_data(function_id: int) -> tuple[np.ndarray, np.ndarray, str]:
    ...
    # Search priority: week-12 > week-11 > week-10 > ...
    search_dirs = [
        repo_root / "week-12" / "data" / f"function_{function_id}",
...
def write_outputs(results: list[dict]) -> tuple[Path, Path]:
    week13_root = Path(__file__).resolve().parents[1]
    submission_path = week13_root / "submission_round13_nn.txt"
    diagnostics_path = week13_root / "doc" / "structure_strategy_justification.md"
    ...
    for r in results:
        ...
        lines.append(
            "- Build on first twelve rounds: query uses cumulative history and "
            "targets recurring strong regions while keeping the search space diversified"
        )
        mse = r["nn_metrics"]
        lines.append(
...
def main() -> None:
    ...
    print("Week 13 structure-guided NN query summary")
    print("=" * 60)
    for r in results:
        mse = r["nn_metrics"]
