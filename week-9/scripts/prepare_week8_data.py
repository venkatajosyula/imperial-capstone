"""
Parse week-9/inputs.txt and week-9/outputs.txt (cumulative 8-round data),
then append only the round-8 result to week-7's cumulative .npy files,
saving the result into week-8/data/function_X/.
...
def main() -> None:
    ...
    week9_dir = repo_root / "week-9"
    week8_data_root = repo_root / "week-8" / "data"

    raw_inputs = (week9_dir / "inputs.txt").read_text(encoding="utf-8").splitlines()
    outputs_lines = (week9_dir / "outputs.txt").read_text(encoding="utf-8").splitlines()
    ...
    # week-7/data already contains rounds 1-7 (17 points for 2D functions).
    # Only the last row (round 8) is new and must be appended.
    ...
    for fid in range(1, n_functions + 1):
        ...
        # Load existing cumulative data from week-7
        week7_dir = repo_root / "week-7" / "data" / f"function_{fid}"
        x_existing = np.load(week7_dir / "inputs.npy")
        y_existing = np.load(week7_dir / "outputs.npy")
        ...
        out_dir = week8_data_root / f"function_{fid}"
