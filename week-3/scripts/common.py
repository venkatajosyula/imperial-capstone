...
def load_function_data(function_id: int) -> tuple[np.ndarray, np.ndarray, str]:
    ...
    Priority order:
    1) week-3/data/function_N with common cumulative naming
    2) week-2/data/function_N with common cumulative naming
    ...
    for x_name, y_name in candidate_pairs:
        ...
        if x_path.exists() and y_path.exists():
            return np.load(x_path), np.load(y_path), f"week-2/data/function_{function_id} fallback ({x_name}, {y_name})"
    ...
    raise FileNotFoundError(
        ...
        "Expected files under week-3/data/function_N, week-2/data/function_N, or week-1/initial_data/function_N."
    )
